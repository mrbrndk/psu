...
GLint size = SIZE_SMALL;//9x9 11x11 15x15
...
int freefields = SIZE_SMALL * SIZE_SMALL;//изначально все свободны
...
void CreateGame()
{
	for (int i = 0; i < SIZE_BIG; i++)
		for (int j = 0; j < SIZE_BIG; j++) table[i][j] = 0;
	srand(time(NULL));
	int count = 0;
...
void CheckRecord(HWND hWnd)
{
	...
	{//если счет больше хотя бы последнего рекорда
		for (int i = 0; i < RECORD_AMOUNT; i++)
		{
			...
			{
				for (int j = RECORD_AMOUNT - 2; j >= i; j--)
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_CREATE:
		{
			...
			{
				WCHAR buff[85];
				if (ReadFile(hFile, (LPVOID)buff, sizeof(buff), nullptr, NULL))
				{
					buff[84] = '\0';
					if (wcscmp(buff, check) == 0)
					{
						...
						{
							if (ReadFile(hFile, (LPVOID)&tmp, sizeof(Record), nullptr, NULL))
								records[i] = tmp;
						}
					}
				}
...
INT_PTR CALLBACK NewGame(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_COMMAND:
		{
			...
			{
				if (Button_GetCheck(GetDlgItem(hDlg, IDC_RADIO9))) size = SIZE_SMALL;
				if (Button_GetCheck(GetDlgItem(hDlg, IDC_RADIO11))) size = SIZE_MIDDLE;
				if (Button_GetCheck(GetDlgItem(hDlg, IDC_RADIO15))) size = SIZE_BIG;
...
INT_PTR CALLBACK Settings(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			int choice = 0;
			if (size == SIZE_SMALL) choice = IDC_RADIO9;
			else if (size == SIZE_MIDDLE) choice = IDC_RADIO11;
			else if (size == SIZE_BIG) choice = IDC_RADIO15;
		...
		case WM_COMMAND:
		{
			if (LOWORD(wParam) == IDOK)
			{
				...
				if (Button_GetCheck(GetDlgItem(hDlg, IDC_RADIO9)) && size != SIZE_SMALL) size = SIZE_SMALL;
				else if (Button_GetCheck(GetDlgItem(hDlg, IDC_RADIO11)) && size != SIZE_MIDDLE) size = SIZE_MIDDLE;
				else if (Button_GetCheck(GetDlgItem(hDlg, IDC_RADIO15)) && size != SIZE_BIG) size = SIZE_BIG;
				else is_resized = false;
				if (is_resized)//если размер поля изменился то, создаем новую игру
				{
					Resize(GetParent(hDlg));
					CreateGame();
				}
